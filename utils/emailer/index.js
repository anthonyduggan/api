...
module.exports.send = async function(to, from, email_type, data) {
    const subject = SUBJECTS[email_type] || DEFAULT_SUBJECT;
    if (config.get('NODE_ENV') !== 'tests') {
        debug(`Sending '${subject}' to ${to}`);
        let html = '';
        try {
            html = await ejs.renderFile(path.join(__dirname, 'templates', `${email_type}.ejs`), data);
        } catch(err) {
        ...
        let text = '';
        try {
            text = await ejs.renderFile(path.join(__dirname, 'templates', `${email_type}_text.ejs`), data);
        } catch(err) {
            console.log(err); // eslint-disable-line
